...
SickQL.Implementations = SickQL.Implementations or {
  ...
  ['TMySQL'] = {
    ...
    Connect = function(db)
      local _, err = db.VendorDatabase:Connect()
      if err ~= nil then
    ...
    Query = function(vdb, q)
      vdb:Query(q.String, function(res)
        if res[1].error ~= nil then
          q:OnError(res[1].error)
          return
        end

        q:OnSuccess(res[1].data)
      end)
    end,
    Disconnect = function(vdb, conn)
...
---@return table
---@return string?
function SickQL:New(vendor, host, port, username, password, database)
  local db = setmetatable({
    Implementation = self.Implementations[vendor],
    VendorDatabase = nil,
    ConnectionInfo = {
      Host = host,
      Port = port,
